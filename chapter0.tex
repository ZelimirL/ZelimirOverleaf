\chapter{Charge Transfer Processes}
\label{chap:RRP}
\section{Description}
...
As stated we will focus on a charge transfer processes in low energies, i.e. ultra-cold temperatures. Recently the advances in ultra-cold atom processing technologies have made is feasible to explore the behaviour of ultra-cold atoms and ions in laboratory. Researches are able to explore the interactions between coole ions and neutral atom, including non-radiative and radiative charge transfer as well as radiative association, in which ions and atoms re-combine and re-arrange at cold temperatures. The applications of cold atom chemysty are in astrophysical application, and buffer gas cooling of single ion clocks. The most recent and quite interesting application is the quantum computing. The approach is to  start from identical atoms, cool them to nanokelvin temperatures, and control them with lasers and magnetic fields. However the entaglement is only stable at ultra cold temperatures (milli and nano Kelvins).
...
\item Phase shifts $ \longleftrightarrow $  Entaglement phases
...
We shall here briefly describe resonant and radiative charge transfer processes as well as the radiative association processes.
...
In our case proces is $ H^+ + H \rightarrow H + H^+ + \gamma $. \\
...
There is also emission of a photon, but as a result a molecule is formed. In this case a photon removes excess electronic energy and the relative kinetic energy. So in this case, from a  wavefunction point of view, there is a transition from a continuoum state to a bound vibrational–rotational state.

This proces is dominant at very low collision energies. This is a key mechanism for molecule formation in the interstellar medium.
...
\subsection{Classical Treatments}
...
Following the approach in the \cite{ZL} we apply a semi-classical approach to the system of two $ H_2^{+} $ molecules. In this approach, we assume that a photon is emitted with energy equal to the energy difference between two Born-Oppenheimer potential surfaces, at the distance where the transition occurs. This leads to the Local Optical Potential Method, and following the semi-classical approach, the total rate is estimated as a classical integral over all localized transitions.

\section{Radiative Association Theory}
\label{chap:RxRP}

Here we follow the discussion in \cite{ZL}, and apply the theory to the 3D Hydrogen atom.
...
This proces is governed by the selection rules, 
\begin{itemize}
...
\begin{figure}[H]
\centering
  \includegraphics{RadAssocPotentials.png}
\end{figure}
...
The proces is $ H^+ + H \rightarrow H + H^+ $.
...
Here we shall investigate the low temperature collision process between the hydrogen atom and the hydrogen ion, in 2 dimension. We use fully quantum mechanical approach to calculate the cross section and the emission spectra of the reaction $ H(2\prescript{2}{}S) + H(1\prescript{1}{}S)\rightarrow H(1\prescript{1}{}S) + H(1\prescript{1}{}S) + \hbar\omega $, formed by the quenching of the excited $ H $ atom by the approaching $ H $ atom, in 2D dimensions. To my knowledge, there is no experimental results related to the 2D problem. There are results for the 3D case, listed in \cite{Zygelman88} and references there.

n this calculation, the atoms are confined in 2 dimensions, while the radiation is emitted or absorbed in all 3D space. Therefore the photon can be emitted in any direction and the potential felt by the incoming ion is the standard 3D Coulomb potential.
