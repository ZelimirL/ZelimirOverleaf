...
Following the approach in the \cite{ZL} we apply a semi-classical approach to the system of YbCa molecules. In this approach, we assume that a photon is emitted with energy equal to the energy difference between two Born-Oppenheimer potential surfaces, at the distance where the transition occurs. This leads to the Local Optical Potential Method, and following the semi-classical approach, the total rate is estimated as a classical integral over all localized transitions.
...
\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{RadAssocPotentials-4.png}
...
\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{EnsteinA3D.png}
...
We have made the calculations two ways. First starting from the equation \eqref{eq4} we compututed the cross section using equation \eqref{sumZZ}. In the figure \ref{compFig3} symbol $ X $ represents the upper limit for total radiative relaxation, which is obtained by adding the association cross sections \eqref{sumZZ} with those given by expression \eqref{sigmaUpper}.\\
...
\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{CrossSection3D.png}
...
hough the optical potential method provides a good approximation for the total radiative relaxation rate, calculation of the photon emission spectrum requires the use of the FGR method. use of the FGR method. In \ref{Fig5} we illustrate the association cross sections $ \sigma_{n\,J} $ at $ T = 1mK $ for the individual rho-vibrational levels as function of the frequency of the emitted photon. The structure of the emission pattern is the result of the oscilation in the incoming wave as shown in \ref{RadAssoc4}

\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{Spectrum3D.png}
