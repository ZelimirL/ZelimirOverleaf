...
At ultralow temperatures ($ \mu K $K and below), only S-wave is present rates obeys Wigner threshold law. In that case, if a Yb isotope supports a bound state just below threshold, the scattering length becomes very large, the radiative association rate is strongly enhance. The enhacement becomes isotope specific. This explains the pronounced isotope separation seen in calculated rate curves as shown in \cite{ZygEtAll4}.
