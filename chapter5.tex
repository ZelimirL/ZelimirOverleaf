...
\end{split} 
\end{equation}
where $ V_{g,u}(R) $ represents the potential for the gerade (g) and ungerade state (u)
...
In the calculation for the phase shifts, or $b_{m}$, we use the accurate $g,u$ potential curves calculated using the method outlined in Chap. (xx)
However, for large internuclear separations $ R \approx > 13 a_{0} $, that numerical method becomes 
unstable. Instead, we match those calculated curves with the analytic asymptotic form described
below.
...
scattering coefficients for the gerade and ungerade potentials, respectively.
 ...
 The following is the plot of the charge transfer cross section in a.u. vs energy. We did a calculation for the range of  $ 10 \mueV $ to $ 1 meV $ which corresponds to temperatures from $ 0.11 K $ to $ 11 K $.
...
{ \Large Calculate the RCT cross sections for only a few partial waves, e.g., the first 5 partial waves. Pick a low collision energy so only the first 5 partial wave contributions to the RCT cross section. Plot the individual partial cross sections. }
