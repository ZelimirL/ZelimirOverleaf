...
scattering coefficients for the gerade and ungerade potentials, respectively.
 ...
 charge transfer cross section. We compute it in the Wolfram Mathematica code listed in appendix \ref{AppendixE}.

 In the figure \ref{fig:CTCS} below the plot of the charge transfer cross section in a.u. vs energy. We did a calculation for the range of  $ 10 \mu eV $ to $ 1 meV $ which corresponds to temperatures from $ 0.11 K $ to $ 11 K $.

 \begin{figure}[h]
   \centering
   \includegraphics{CTCrossSection1}
   \caption{Charge Transfer Cross Section corresponding to the first 5 partial waves}
   \label{fig:CTCS}
 \end{figure}

We also calculated the resonant charge transfer cross for each of the first 5 partial waves. The following graphs show the cross section for the same energy (temperature) range. 
...
We expand the discussion in the following section.

\section{Role of Symmetry and Anyonic Flux tube}

The nuclei of the H molecule are identical fermions, and so we enforce standard symmetry requirements for the latter. Symmetrization requires the inclusion of the spin degree of freedom, and here, we assume that both nuclei are spin- polarized. 

Therefore, under traditional interpretations in which the nuclei are Fermions, we take the spatial component of the wave amplitude to be anti-symmetric under interchange $ P_{12}\pmb{R}_{1} \rightarrow \pmb{R}_2  $ or $ P_{12}\pmb{R} \rightarrow -\pmb{R}  $ where $ P_{12} $ is an interchange operator. Now since
\begin{equation}
\begin{split}
  &  F = \psi_g(\pmb{R})\phi_g + \psi_u(\pmb{R})\phi_u \\[.8em]
  &  P_{12}F = \psi_g(-\pmb{R})\phi_g + \psi_u(-\pmb{R})\phi_u
\end{split}
\end{equation}
We symmetrize by requiring the amplitude
\begin{equation}
F \rightarrow (\psi_g(\pmb{R}) - \psi_g(-\pmb{R}))\phi_g + (\psi_u(\pmb{R}) - \psi_u(-\pmb{R}))\phi_u
\end{equation}
  or
\begin{equation}
\begin{split}
  &  F \rightarrow \sqrt{2}(e^{ikR\cos\phi} + e^{-ikR\cos\phi})\phi_a + \sqrt{2}(f_g(\phi) - f_g(\phi + \pi) + f_u(\phi) - f_u(\phi + \pi))\phi_a \\[.8em]
  & + \sqrt{2}(f_g(\phi) - f_g(\phi + \pi) - f_u(\phi) + f_u(\phi + \pi))
\end{split}
\end{equation}
Thus
\begin{equation}
\sigma_{RCT} = \frac{1}{8}\int_0^{2\pi}{\left|f_g(\phi) - f_g(\phi + \pi) - f_u(\phi) + f_u(\phi + \pi)\right|} = \frac{2}{k}\sum_{m=\pm 1, \pm 3, ...}{\left|b_m^g - b_m^u\right|^2}
\end{equation}
In a pioneering paper, J. M. Leinass and J. Myrheim \cite{LeinassMyrheim} argued that in two dimensions, identical particles can exhibit interchange statistics that differ from Bosons or Fermions. As emphasized, the topology of the configuration space of identical particles in a 2D spatial manifold differs from that of a 3-dimensional manifold. The latter allows the amplitude $ \psi(\pmb{r}_1,\pmb{r}_2) $ for the pair of identical particles to obey $ P_{12}\psi(\pmb{r}_1,\pmb{r}_2) = e^{i\alpha}\psi(\pmb{r}_2,\pmb{r}_1) $ where $ \alpha $ is an arbitrary constant.
For Fermions $  \alpha = \frac{1}{2} $, and for Bosons $  \alpha = 1 $.

Particles that differ from Fermion/Boson statistics are called Anyons \cite{Wilzceck1982}. Whether such particles exist in nature is of topical and intense investigation. Several studies have examined the role of the latter in the bulk properties of an Anyon gas \cite{AROVAS1985117}. There are two popular methods used to enforce Anyonic statistics among identical particles.  The first involves positing a Chern-Simons \cite{IENGO1992179} term that couples each anyon to a pure gauge field. Alternatively, one posits the presence of an Aharonov-Bohm infinitesimal magnetic flux tube piercing the Anyon at an angle perpendicular to the 2D plane(see Fig \ref{AHBAnyon}).

\begin{figure}[ht]
  \centering
  \noindent\includegraphics[width=\linewidth]{Ahb-1}
  \caption{Aharonov - Bohm configuration, the flux tube must be perpendicular to the plane}
  \label{AHBAnyon}
\end{figure}

We shall assume the latter for the nuclei of our collision partners. Hence, the Hamiltonian for a pair of 2D $ H $ atoms, whose
nuclei are Anyons, takes the form
\begin{equation}
  H = \frac{1}{2m}(\pmb{p}_1 - \pmb{A}) + \frac{1}{2m}(\pmb{p}_2 - \pmb{A}) + H^{'}(\pmb{r}_1, \pmb{r}_2)
\end{equation}
where $ H^{'}(\pmb{r}_1, \pmb{r}_2) $ includes the electronic kinetic energy an all Coulombic interac-
tions discussed in the previous chapters.
\begin{equation}
A \equiv \frac{\alpha}{2\pi}\frac{n \times (\pmb{r}_1 - \pmb{r}_2)}{\left|\pmb{r}_1 - \pmb{r}_2\right|^2}
\end{equation}
is a gauge potential \cite{YSWU}. Here $ \pmb{n} $ a unit normal vector to the 2D scattering plane. Introducing the relative coordinates
\begin{equation}
\begin{split}
  & \pmb{R} = \pmb{r}_2 - \pmb{r}_1 \\[.8em]
  & R_{CM} = \frac{1}{2}(\pmb{r}_2 + \pmb{r}_1)
\end{split}
\end{equation}
The Schrodinger equation for the relative motion $ F(\pmb{R}) $ becomes
\begin{equation}
  -\frac{1}{2\mu}(\nabla - i\pmb{A})^2F(\pmb{R}) + V_{g,u}(R)F(\pmb{R}) - EF(\pmb{R}) = 0
\end{equation}
where $ \nabla $ is the 2D gradient operator and, in a cylindrical coordinate system,
\begin{equation}
  \pmb{A} = \frac{\hat{\phi}}{2\pi}\frac{\alpha}{R}
\end{equation}
or
\begin{equation}
\frac{d^2}{dR^2}F(\pmb{R}) + \frac{d}{dR}F(\pmb{R})\frac{1}{R} + \frac{1}{R^2}\left(\frac{\partial}{\partial \phi}-i\frac{\alpha}{2\pi}\right)^2 F(\pmb{R}) + (k^2 - 2\mu V_{g,u}(R))F(\pmb{R}) = 0  
\end{equation}
where $ k $ is the wave number. $ \mu $ is the reduces mass,  and $ V_{g,u}(R) $ in the BO potential for the gerade $ g $ and ungerade $ u $ state.

Now that if we impose a singular gauge transformation $ F(\pmb{R}) \rightarrow e^{i\alpha \phi 2/pi} \hat{F(\pmb{R})} $ then $\hat{F(\pmb{R})} $ obeys the standard Schrodinger without the presence of a flux tube. However, the pre-factor $e^{i\alpha \phi 2/pi} $ introduces multi-valuedness in the total amplitude and special methods must be invoked to cancel that singularity. Set ting aside the question of multi-valuedness, the above feature highlights the difference between the behavior of the amplitude under particle interchange $ \pmb{R} \rightarrow -\pmb{R} $. and the introduction of an Aharonov-Bohm flux tube.\\

Here, we follow the procedure of Aharonov-Bohm and expand $ F(\pmb{R}) $ in terms of single-valued function $ e^{im\phi} $ , $ m = 0, \pm 1, ... $. resulting in the the radial equations \eqref{AHBeq1} (Aharonov-Bohm radial equation)
\begin{equation}\label{AHBeq1}
  F_m^{''}(R) + \frac{F_m^{'}(R)}{R} - \frac{(m - \alpha)^2}{R^2}F_m(R) + (k^2 - 2\mu V_{g,u}(R))F_m(R) = 0
\end{equation}
from which we construct the amplitude in the asymptotic region
\begin{equation}\label{sumampl}
  \sum_{m=0,\pm 1}{e^{im\phi}\left(a_mJ_{\left| m-\alpha\right| }(kR) + b_mH_{\left| m-\alpha\right| }^{(1)}(kR)\right)}
\end{equation}
If we choose (see Aharonov Bohm) $ a_m = (-i)^{\left| m- \alpha \right| } $  the above expression reduces 
\begin{equation}
F_{g,u}\rightarrow e^{i\alpha\phi}e^{-ikz} + \frac{e^{ikR}}{\sqrt{2\pi kR}}f_{AB}(\phi) + f_{g,u}(\phi)\frac{e^{ikR}}{\sqrt{R}}
\end{equation}
where $ f_{AB}(\phi) $ is the Aharonov-Bohm scattering amplitude, and $ f_{g,u}(\phi) $ is given by the expression
\begin{equation}\label{ahbw}
  f_{g,u}(\phi) = \sqrt{\frac{2}{\pi k}}\sum_{m=\pm 1,\pm 3}{(-i)^{\left| m- \alpha \right| }e^{im\phi}b_m^{g,u}}
\end{equation}
where
\begin{equation}\label{bgu2}
  b_m^{g,u} = (-i)^{\left| m- \alpha \right| }\frac{J_{\left| m- \alpha \right| }^{'}-y_m^{g,u}J_{\left| m- \alpha \right| }}{H_{\left| m- \alpha \right| }^{'} - y_m^{g,u}J_{\left| m- \alpha \right| }}
\end{equation}
and the cross section becomes
\begin{equation}
  \sigma_{CT} = \frac{2}{k}\left(\sum_{m=\pm 1, \pm 3}{\left| b_m^{(g)} - b_m^{(u)}\right|^2}\right)
\end{equation}
and $ J^{'}, H^{'} $ represent derivatives with respect to $ R $, and $ y^{g,u} $ the log-derivates corresponding to gerade and ungerade potentials respectively. Following the discussion leading to \eqref{sumampl}, we obtain the outgoing wave of the charge transfer amplitude given by \eqref{ahbw}, but where $ b_m^{g,u} $ satisfies relation \eqref{bgu2}, instead of \eqref{bm2DReg}. Note that the terms proportional to $ f_{AB}(\phi) $ cancel as they are independent of the gerade/ungerade potentials. The restriction to odd values $ m $ is determined by the fermionic symmetry requirement of the base $ \alpha = 0 $ system.
