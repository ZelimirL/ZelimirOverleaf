...
\end{equation}
and $ J^{'}, H^{'} $ epresent derivatives with respect to $ R $, and $ y^{g,u} $ the log-derivates corresponding to gerade and ungerade potentials respectively. Following the discussion leading to Eq. (xx), we obtain the outgoing wave of the charge transfer amplitude given by Eq. (xx), but where $ b_m^{g,u} $ satisfies relation (xx), instead of (xx). Note that the terms proportional to $ f_{AB}(\phi) $ cancel as they are independent of the gerade/ungerade potentials. The restriction to odd values $ m $ is determined by the fermionic symmetry requirement of the base $ \alpha = 0 $ system.
