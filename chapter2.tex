...
\begin{figure}
 \captionsetup{type=figure}
  \includegraphics[alt={Coordinate diagram for the two-dimensional hydrogen molecular ion. Two protons are separated by R along the horizontal axis, and the electron lies in the plane at distances r sub a and r sub b from them.}]{H2Ion3D-2}
  \caption{Hydrogen molecular ion in 2 dimensions} 
  \label{h2ion2d}
\end{figure}
it follows:
\begin{equation}
...
(1 - \mu^2)L\frac{\partial^2 F}{\partial \mu^2} - \mu L\frac{\partial F}{\partial \mu} \right]\\[.8em]
& + \frac{4}{R}\frac{\lambda}{\lambda^2-\mu^2} F L + E F L = 0 \Rightarrow \\[.8em]
...
The Mathematica code is in appendix \ref{AppendixC}. The energies calculated and the plot of the potential for the states $1s_g^{+}$ and $ 1s_u^{+} $ are in the tables \ref{1sg} and \ref{2sg}

The states  $ 1s_g $ and $ 1s_u^{+} $ represent the Gerade ground state and the Ungerade state respectively.

    \begin{table}[h!]
  \caption{The separation constant A, electronic energy E, and total energy $E+1/R$ for the $1s_g$ state at selected internuclear separations R}
  \centering
  \label{1sg}
...
\begin{table}[h!]
  \caption{The separation constant A, electronic energy E, and total energy $E+1/R$ for the $1s_u^{+}$ state at selected internuclear separations R}
  \centering
  \label{2sg}
		\tagpdfsetup{table/header-rows={1}}
		\begin{tabular}{ m{6em} m{6em}  m{6em}  m{6em} m{6em} }
...
\begin{figure}
  \includegraphics[width=\textwidth,alt={Electronic energy and total potential versus internuclear distance for the first gerade state. The total potential has a pronounced minimum near R equals 0.7 and approaches minus 2 Hartree from below.}]{H2_1Sg-Potential}
  \caption{$ H_2^{+} $ energy and potential curves for the $ 1s_g^{+} $ state}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth,alt={Electronic energy and total potential versus internuclear distance for the first ungerade state. The total potential decreases monotonically toward minus 2 Hartree and does not form a bound-state minimum.}]{H2_1Su-Potential}
  \caption{$ H_2^{+} $ energy and potential curves for the $ 1s_u^{+} $ state}
\end{figure}


\begin{figure}
  \includegraphics[alt={Comparison of first gerade and first ungerade molecular potentials versus internuclear distance. The gerade curve has a deep minimum near R equals 0.7, while the ungerade curve is repulsive; both approach minus 2 Hartree at large R.}]{H2_sGpU-Potential}
  \caption{$ H_2^{+} $ potential curves for the first gerade (red) and first ungerade (green) states}
\end{figure}

For comparison we used paper by Patil \cite{H2Plus2d1}. Our result are very close to the results obtained by Patil.
Also one can observe that in 2 dimensions the max depth of the potential well of the $ H_2^{+} $ ground state is approx $ V(R=0.5) = -2.83822 $ a.u. compared to the $ V(R=0.4) = -3.63220 $ in the 3D case, which we showed in table \ref{tab:UsVsBates1s}
Also the shape of the potential well is more compact in the 2D case, for the ground $ 1s_g^{+} $ gerade state. For the ungerade $ 1s_u^{+} $ state the situation is even more different. The potential curve of the  $ 1s_u^{+} $ state does not really have a minimum for and does not allow a bound state.
...
In the tables below we show the accurate and LCAO values for the potential of the gerade $ 1sG $ table \ref{tableG} and ungerade $ 1sU $ table \ref{tableU} states.

\begin{table}[ht]
  \caption{Accurate and LCAO energy as function of R (internuclear distance) for the gerade state}
  \centering
  \label{tab:lcaotabG}
  \tagpdfsetup{table/header-rows={1}}
...
\begin{table}[ht]
  \caption{Accurate and LCAO energy as function of R (internuclear distance) for the ungerade state}
  \centering
  \label{tab:lcaotabU}
  \tagpdfsetup{table/header-rows={1}}
  \begin{tabular}{ m{4em} m{4em} m{4em} }
