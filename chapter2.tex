...
The Mathematica code is in appendix \ref{AppendixC}. The energies calculated and the plot of the potential for the states $1s_g^{+}$ and $ 2p_u^{+} $ are in the tables \ref{1sg} and \ref{2sg}
...
Also the shape of the potential well is more compact in the 2D case, for the ground $ 1s_g^{+} $ gerade state. For the ungerade $ 2p_u^{+} $ state the situation is even more different. The potential curve of the  $ 2p_u^{+} $ state does not really have a minimum for and does not allow a bound state.
