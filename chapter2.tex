...
\section{Exact solution of the electron energies of the $ H_2^+ $ molecule}
...
Plug the coordinate above we finally get from \ref{AppendixG}.
...
p^2 = -\frac{E\,R^2}{2}
\end{equation}
Following the derivation in \ref{AppendixG}
...
\subsubsection{Mostly Algebraic Solution}

Since $ M(\mu) $ is Mathieu's equation, we can exploit the eigenvalue relations for the Mathieu's equation. Following \cite{H2Plus2d2}, the complete solution is described in appendixC. The appendixG also contains a Wolfram Mathematica code used to obtain a values for electronic energy $ E(R) $, total potential $ V(R) $ for the  $ 1s_g^{+} $ and $2p_{s}^{+} $ states. 
