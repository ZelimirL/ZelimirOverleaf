...
In actual calculation we first start by computing the phase shift $ \delta $ from equation \eqref{remoteWave} which we repeat here.
\begin{equation}\label{remoteWave2}
  g^{+}_{m}{(kR)} \sim A_m(kR)^{-1/2}\cos\left[kR - \pi\frac{m+1/2}{2} + \delta_m\right],\,\,{R \rightarrow \infty}
\end{equation}
$ A_m $ is a constant.
...
The logarithmic derivative of \eqref{remoteWave}:
\begin{equation}
\begin{split}
  & \frac{d}{dR}\ln(g^{+}_{m}(kR)) = \\[.8em]
  & = \frac{-(1/2)k^{(-1/2)}A_m R^{-3/2}\cos\left[kR - \pi\frac{m+1/2}{2} + \delta_m\right]-kA_m(kR)^{-1/2}\sin\left[kR - \pi\frac{m+1/2}{2} + \delta_m\right]}{A_m(kR)^{-1/2}\cos\left[kR - \pi\frac{m+1/2}{2} + \delta_m\right]}
\end{split}
\end{equation}

\begin{equation}
  \frac{d}{dR}\ln(g^{+}_{m}(kR)) =  -\frac{1}{2R} - k\tan(kR - \pi\frac{m+1/2}{2} + \delta_m) 
\end{equation}

Now this is going yo be matched by the logarithmic derivative of the wavefunction inside the scattering region. Since that function cannot be obtained analytically we need to use numerical methods. So using Mathematica we solve the wave equation \eqref{frrSr} in the scattering potential and perform numberical logarithmic derivative.

TODO: add appendix
The Mathematica code is in Appendix
