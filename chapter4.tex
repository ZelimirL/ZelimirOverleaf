...
In Table \ref{tab:lcaotabG} we show the accurate vs LCAO values for the gerada state.
...
We now show the same comparison for the ungeradate state in Table \ref{tab:lcaotabU} we show the accurate vs LCAO values for the ungerada state.
...
\begin{equation}
\begin{split}
  & \lambda = \frac{1}{4}\int_{0}^{2Pi}d\theta\frac{\left|f_g(\theta) - f_u(\theta)\right|^2}{4Pi} \Rightarrow \\
...
It is now fairly straightforward to compute the scattering length. We compute the scattering length in the Wolfram Mathematica code listed in \ref{AppendixE}.

The following is the plot of the scattering length in a.u. vs energy. We did a calculation for the range of  $ 10meV $ to $ 1 eV $

\includegraphics{CrossSection1.png}

The Mathemnatica code is in appendix \ref{AppendixF}
...
The following groph shows the Einsten A coefficient as function or the internuclear distance $ R $.
\includegraphics{"Einstein.png"}

The relevant code is in \ref{AppendixG}
...
The preceding form of the 2D Schrodinger equation and its solution in 2D remain the same.

Finally the total cross section changes:
\begin{equation}
  \sigma^{anyon} = \frac{4}{k}\sum_{m=-\infty}^{+\infty}{\sin^2(\delta_{m+\beta})}
\end{equation}

The charge transfer cross section becomes:
\begin{equation}
  \sigma^{anyon} = \frac{4}{k}\sum_{m=-\infty}^{+\infty}{\left|\delta_{m+\beta}^{+}-\delta_{m+\beta}^{-}\right|}
\end{equation}
