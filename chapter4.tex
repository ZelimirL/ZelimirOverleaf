...
So following the \cite{Dalgarno1953} we have the $ |R| \leftarrow \infty $ asymptotical eigenstates of equation \eqref{hdia}, both gerade and ungerade:
\begin{equation}
\begin{split}
...
\begin{equation}
  F_g(R) \underset{R \rightarrow +\infty}{\sim} e^{ik\ x} + f_g(\lambda)\frac{e^{ik\ x}}{\sqrt{R}} 
\end{equation}
Now we use the 2D scatering amplitude from the previous chapter:
\begin{equation}\label{fgampl}
  f_g(\lambda) = \sqrt{\frac{2}{Pi}}\sum_{m=0}^{\infty}{\epsilon_m \cos(m\lambda)e^{i\delta_m^g}\sin\delta_m}
\end{equation}
As the $ f(\lambda) $ is not directly observable, we look for the cross section. And we get for the differential cross section, in 2D, we have:
\begin{equation}
  \lambda(\lambda) = |\sqrt{i}{k}f_g(\lambda)|^2 = \frac{1}{k}|f_g(\lambda)|^2 
\end{equation}
and the total cross section
\begin{equation}
\begin{split}
  & \lambda_g = \int_{0}^{2Pi}{d\lambda f_g(\lambda)} = \frac{2}{Pi}\sum_{m,m'}\int_{0}^{2Pi}{\epsilon_m\epsilon_m' \cos(m\lambda)\cos(m'\lambda)e^{i\delta_{m}^g}e^{i\delta_{m'}^g}\sin\delta_m^g\delta_{m'}^g} = \\
  & = \frac{2}{Pi}\sum_{m}{\epsilon_m\sin^2\delta_m^g}
\end{split}
\end{equation}
The same dicusion applies for the ungerade solution so we :
\begin{equation}
  \lambda_u = \frac{2}{Pi}\sum_{m}{\epsilon_m\sin^2\delta_m^g}
\end{equation}

In our case we are not interested in the elastic scattering of the gerade and uungerade states,as they form the linear combination of the asymptotic orbitals for the atom A and B, respectively.

Instead we seek the asymptotic form for the difference
\begin{equation}
  \frac{1}{2}\left(F_g(R) - F_u(R)\right) \sim \frac{\left|f_g(\lambda) - f_u(\lambda)\right|}{4Pi}
\end{equation}
Using expression \eqref{fgampl} we get:
\begin{equation}
\begin{split}
  & lambda = \frac{1}{4}\int_{0}^{2Pi}d\lambda\frac{\left|f_g(\lambda) - f_u(\lambda)\right|^2}{4Pi} \Rightarrow \\ 
...
We can express this function in the partial waves bases. Since $ V_b $ contains no bound states we get for $ \mathbf{R} = \mathbf{R}(R,\lambda) $:
\begin{equation}\label{GreenFka2}
\begin{split}
G^{+}(\mathbf{R}, \mathbf{R}') =  \frac{\pi\mu}{k_b}\sum_{l=0}^{\infty}{\sqrt{\frac{1}{2\pi}} P_l(\cos\lambda) P_l^{*}(\cos\lambda^{'})}\times \frac{f_l(k_bR_{<})g^{+}_l(k_bR_{>})}{RR'}
\end{split}
\end{equation}
where $ P_l(\cos\lambda) $ are Legendre polynomials, also $ \lambda = 0 $ part of the spherical harmonics:  $ P_l(\cos\lambda) = Y_{l0}{\lambda,0} $. 
...
F_a(\mathbf{R}) = \sum_{m=1}^{\infty}{P_m(\cos\lambda)(2m+1)i^{l}\sqrt{\pi}\times \exp[i\delta_m]\frac{s_m(k_a\mathbf{R})}{k_a\mathbf{R} } } 
\end{equation}

The asymptotic form for \eqref{Flong1} is:
\begin{equation}\label{FlongA}
F_a(\mathbf{R}) \sim \frac{1}{\sqrt{2}}\left[e^{ik_az}-e^{-ik_az} + [f(\lambda,\phi) - f(\lambda-\pi,\phi+\pi)]\frac{e^{ik_aR}}{R}\right]
\end{equation}
...
F_{k\alpha}(\mathbf{R}) \sim \frac{e^{ik_aR}}{R}f_{k\alpha}(\lambda)
\end{equation}
where:
\begin{equation}\label{fkaa1}
f_{k\alpha}(\lambda) = \sum_{m}{\cos(l\,\lambda)\left[\sum_{m=1}^{\infty}{\frac{2\pi\mu}{k_ak_b}(2m+1)\left(\frac{\pi k c}{A}\right)^{1/2}e^{i\delta_j(b)}e^{i\delta_j(a)}i^{J+l-1} }\right]\sqrt{2l+1)2\pi}}
\end{equation}
...
The cross section of the collision induced transition between the $ H $ atom and the $ H^{+} $ ion is obtained by summing $ \left|f_{k\alpha}(\lambda)\right|^2 $ over all final states that conserve energy with an initial state, and dividing the result with the flux of the incident channel.
...
& = \sum_{\alpha}{\int{\frac{d^2k}{(2\pi)^2}\frac{A}{2\mu k_a}\int{d^2k_b\delta\left[\frac{k_b^2}{2\mu} - \frac{k_a^2}{2\mu} + \Delta E - \hbar\omega \right]\left|f_{k\alpha}(\lambda) \right| } } }
\end{split}
\end{equation}
...
\begin{split}
\text{Im} G^{+}(\mathbf{R},\mathbf{R}) = \pi\sum_{l=0}^{\infty}{\cos(l\, \lambda)\cos(l\,\lambda^{'})\int_0^{\infty}{dk\,\delta\left[\frac{k^2}{2\mu} - \frac{k^2_a}{2\mu} + \hbar\omega - \Delta E \right]\frac{f_l(kR)f_l(kR^{'})}{R\,R^{'}} }  }
\end{split}
\end{equation}
...
\begin{split}
&V(\mathbf{R},\mathbf{R}^{'}) = \frac{i}{2\pi}\sum_{\alpha}\int{d\Omega_k}\int_0^{k_{max}}\sum_{l=0}^{\infty}\cos(l\, \lambda)\cos(l\,\lambda^{'}) \\[.8em]
...
\begin{equation}
\delta^{(2)}\left(\mathbf{R},\mathbf{R}^{'}\right) = \sum_{l=0}^{\infty}{\cos(l\,\lambda)\cos(l\,\lambda')\frac{\delta(R - R')}{R\,R'}}
\end{equation}
