...
\begin{equation}
  \lambda(\theta) = |\sqrt{i}{k}f_g(\theta)|^2 = \frac{1}{k}|f_g(\theta)|^2 
\end{equation}
and the total cross section
\begin{equation}
\begin{split}
  & \lambda_g = \int_{0}^{2Pi}{d\lambda f_g(\lambda)} = \frac{2}{Pi}\sum_{m,m'}\int_{0}^{2Pi}{\epsilon_m\epsilon_m' \cos(m\theta)\cos(m'\theta)e^{i\delta_{m}^g}e^{i\delta_{m'}^g}\sin\delta_m^g\delta_{m'}^g} = \\
