...
The next step is to calculate the phase shift and from the phase shift obtain the scattering length.

\section{Phase Shift Calculation}

We follow the approach in \cite{Dalgarno1953} and in the following expression for the scattering length we compute phase shifts for both gerade and ungerade potentials. The $ \delta_m+ $ and the $ \delta_m^- $ are the phase shift for the gerade and ungerade potential respectively.

So for the purely elastic single-channel 2D cross section, we have Scattering length $\lambda $:
\begin{equation}
\begin{split}
