...
\end{equation}
As the $ f(\theta) $ is not directly observable, in 2D we look for the scattering length. And we get for the differential scattering length, in 2D, we have:
\begin{equation}
  \theta(\theta) = |\sqrt{i}{k}f_g(\theta)|^2 = \frac{1}{k}|f_g(\theta)|^2 
\end{equation}
and the total scattering length
\begin{equation}
\begin{split}
...
It is now fairly straightforward to compute the scattering length. We compute the scattering length in the Mathematica code listed in appendix E.
