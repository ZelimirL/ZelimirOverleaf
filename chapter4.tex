...
where m is the reduced mass, V is a short-range local poten tial, E is the relative energy.

Using polar coordinates, setting $ k^2 = 2mE $ , we have the equation
\begin
\frac{1}{r}\frac{\partial}{\partial r}\left(r\frac{\partial\psi(r\theta)}{\partial r}\right)
\end
...
The equation \eqref{scstterL} represents the number of particles scattered between $ \theta $ and $ \theta + d\theta $ per seoond per unit incident flux.
...
\begin{equation}
\begin{split}
  \lambda = \frac{4}{k}\sum_{m=0}^{\infty}{sin^2(\delta_m^{+}-\delta_m^{i}^{i})}
\end{split}
\end{equation}
