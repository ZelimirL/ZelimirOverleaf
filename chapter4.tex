...
\subsection{Calculation, Phae Shift}

In the actual calculation we first start from the form of the radial wave function for the scattered free particle
\begin{equation}\label{remoteWave2}
  R_m(r) = C_m\left[\cos \delta_l J_l(kr) + \sin \delta_l N_l(kr)\right]
\end{equation}
...
The logarithmic derivative of \eqref{remoteWave2}:

\begin{equation}
  \frac{d}{dR}\log(R_m(r) = \frac{\cos \delta_l J_l^{'}(kr) + \sin \delta_l N_l^{'}(kr)}{\cos \delta_l J_l(kr) + \sin \delta_l N_l(kr)}
\end{equation}

Now this is going to be matched by the logarithmic derivative of the wavefunction inside the scattering region. Since that function cannot be obtained analytically we need to use numerical methods. So using Mathematica we solve the wave equation \eqref{frrSr} in the scattering potential and perform numberical logarithmic derivative.
