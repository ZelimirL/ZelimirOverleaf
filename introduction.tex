...
In this thesis, we focus on two distinct applications of CT. In a recent study, published in the authoritative Physical Review Letters \cite{Ref4} Rellergert et al reported an enhanced radiative association rate of the $Yb^{+}$ ion in collisions with $Ca$ at temperatures in the sub-Kelvin regime. The reported rate was about five orders of magnitude larger than previous measurements of similar species
...
Here, we report on our independent investigation of this collision process. Our results, published in \cite{ZL}, find that Langevin behavior persists into the sub-mK region, and predict a CT association rate for the $YbCa^{+}$ molecule that is orders of magnitude smaller than that reported in the cited measurements.  The results of our investigation have prompted the authors of the laboratory study to reconsider the interpretation of their measurements.

However, in carrying out calculations into the ultracold regime in which only a few partial waves contribute, we find dramatic isotopic behavior not reported, to the best of our knowledge, in the current literature. This discovery was highlighted in a Europhysics feature article \cite{Dulieu2014Europhysics} subsequent to our publications. Our calculations are the first of their kind for this collision system, and in the first part of this thesis, we review and elaborate on our calculations. 
...
\begin{equation}
  V(r) = -\frac{q}{r} - \frac{1}{\lvert R - r \rvert}
\end{equation}
...
Potential has its maximum as $ V_{max} = -\left(\sqrt{q} + 1\right)^2/R $. The electron attaches itself to an incoming ion if its binding energy, perturbed by the electron–ion Coulomb energy $ -1/2 - q/R $ becomes greater than $ V_{max} $.
...
\end{split}
w\end{equation}
where $ R $ is the internuclear distance, $ R_c $ is the value where the energy defect $ \Delta E = E_2(R) - E_1(R) $ has a local minimum. The $ E_{1,2} $ are the energies of the two states, $ b $ is the impact parameter as defined in \cite{ZygelmanCT} and $ v $ is the relative velocity of the collision partners.
...
\end{equation}
where $ F_n(R) $ are quantal amplitudes that predict the probability for the system to occupy the n-th electronic states
...
\end{equation}
and we can integrate out the degrees of freedom associated with the electrons and derive an effective Schrödinger description, for the amplitudes $F_{n}(\bf R)$ that describe the motion of the nuclei. In Chapter \ref{chap:RRP}, we will apply this method to calculate 
