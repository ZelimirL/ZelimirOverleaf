\section{Hydrogen Molecular Ion in 3 Dimensions,  the method of Bates}

In this section, we describe first the work done by the Bates et all \cite{Bates1}\cite{Bates2}\cite{Slater}.  We review analytic methods\cite{Bates1}\cite{Bates2}\cite{Slater} used to obtain solutions of the time-independent Schrodinger equation for the hydrogen molecular ion.

As we will apply a similar method to treat the hydrogen molecular ions in 2D,
we first review the methodology of Bates \cite{Bates1}\cite{Bates2} used to calculate the lowest electronic eigenstates and eigenvalues for the $ {\rm H}_2^{+} $ system in 3D. \footnote {In this section we use atomic (Hartree) units, unless otherwise stated.}
...
Bates et al. \cite{Bates1}\cite{Bates2} did not provide numerical codes used in the evaluation of eigenvalues. Instead, they used an expanded fraction method for calculating the latter with the desired level of precision. The paper\cite{Bates2} does not report on the details of those calculations, and so in this thesis we provide an alternative description. We will compare
 the results obtained here with those reported in\cite{Bates2}. Harmony with those reported in\cite{Bates2} will provide 
...
The only instance in which we will avoid using atomic units is during the derivation that involves transforming the molecular Hamiltonian into a Jacobian coordinate system. Since certain terms will include the reduced mass, we aim to maintain the integrity of the units throughout this process.
...
It is convenient to separate the motion of the center of mass from the motion of the point particles. A similar approach can be applied for the quantum case. We show in appendix \ref{AppendixA} that the motion of the center of mass will be a plane wave, which possess a well defined momentum and which is not square integrable. That makes the position of the center of mass equally probable in the all space, and confirms with the Heisenberg uncertainty principle.

In the appendix \ref{AppendixA} we define $ \chi(\mathbf{R}) $ as the wavefunction representing the motion of the nuclei. We then  obtain:
\begin{equation}
...
\begin{figure}
    \noindent\includegraphics[width=\textwidth,alt={Coordinate diagram for the hydrogen molecular ion. Two protons lie on the z axis at minus R over 2 and plus R over 2; an electron is above the axis, at distances r sub a and r sub b from the two nuclei.}]{H2Ion3D-2}
    \caption{$ H_2^{+} $ ion, showing the position of the nuclei at distance R, and a coordinates of an electron}\label{h2ion3d}
...
\begin{equation}
\begin{split}
& H^{'} = T_{CM} + T_{KE} + H_{AD} \,\,\,\,\text{ where motion of the center} \\[.8em]
& \text{of the mass that which be separated from the total energy of the nuclei: } \\[.8em]
& T_{CM} = -\frac{1}{2(2M +m)}\nabla_{CM}^{2} \\[.8em]
...
Since it is a  Sturm-Liouville problem, or more precisely a system of two Sturm-Liouville problems, our approach is fundamentally the same approach as taken by Bates. but it is more practical, as it can better accommodate the existing numerical software. There exist excellent numerical packages (Lapack) \cite{Lapack1} for computing the matrix eigenvalues, and they greatly simplify the actual computation. The Fortran code in appendix \ref{AppendixD} uses Lapack library to compute eigenvalues.

We express both function $ M(\mu) $ and $ L(\lambda) $ using the set of orthogonal functions which satisfy the boundary conditions. We then follow the method of Robyn's. Once we replace the assumed solution into the equation and after some algebra, we obtain effectively two uncoupled, infinite systems of equations, where the unknowns are the coefficients in the solution series. The solution vector depends on the two parameters, $ p $, which is the function of energy and $ A $ which is the separation constant. 
...
We use the same approach for the M equation.
...
In appendix \ref{AppendixB} we present the Mathematica code for calculating electron energies of the molecular $ H_2^{+} $ ion in 3 dimensions using Mathematica eigenvalue solver.
...
\begin{figure}
  \includegraphics[alt={Electron potential energy E of R versus internuclear distance R. The energy rises monotonically from about minus 3.6 at R equals 0.4 toward the separated-atom limit as R increases.}]{Bates3DEofR-2}
  \caption{Electron potential energy as function of internuclear distance R for 3D $ H_2^+ $ }
\end{figure}
...
Electron energy as an eigenvalue of the equations \eqref{bandL} and \eqref{bandM}.
\begin{figure}
  \includegraphics[alt={Total electronic plus nuclear potential energy versus internuclear distance R. The curve falls steeply to a minimum near R equals 1.2 and then rises gradually toward the dissociation limit.}]{Bates3DEofRPlusVofR-2}
  \caption{Total electron potential energy as function of internuclear distance R for 3D $ H_2^+ $, given as $ E(R) + V(R) $}
\end{figure}

Table comparing the electron  energies obtain using our method vs method applied by Bates et al. We chose a subset of values to indicate the overall trend. The $ 1s\sigma_g $ and $ 2p\sigma_u $ represent the ground Gerade and Ungerade state respectively. 
In the table below we used atomic units for both $ R $  and $ E(R) $ .

  \begin{table}[ht]
    \caption{Electron energy, $ 1s\sigma_g $ state, \label{groundEven}}
  \centering
...
For reference we use the values obtained by Bates \cite{Bates1}\cite{Bates2}. Valued obtained by our method are in good alignment with the referent values.
