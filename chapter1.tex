\chapter{Existing Non-Relativistic Solution of the Hydrogen Molecular Ion in 3 Spatial Dimensions}

In this chapter we review the method of Bates et al \cite{Bates1}, and successfully reproduce the results, by finding the lowest eigenvalue of the $ H_2^{+} $ system. \footnote {In Bates paper energy is in atomic (Hatree) units}

The Hydrogen Molecule Ion, and similar systems, ( such as $ HeH^{+} $) represent the simplest molecular system to which we can apply the full quantum mechanical treatment. It is actually the only molecule for which we can solve the electronic Schrodinger equation exactly, although the solution does not exist in the closed form, but rather as an infinite series. The seminal study on Hydrogen Molecular Ion has been done by Bates et all \cite{Bates1}\cite{Bates2}. It has been studied extensively since then, and its solution is part of the molecular quantum theory textbooks \cite{Slater}.  

The subsequent studies and textbooks on the $ H_2^{+} $ systems, rely on the Bates results. So any new approach for solving $ H_2^{+} $ should be able to replicate the Bates' results exactly. Wasserman in \cite{ExperimentalBates2} reports that while the existence of $ H_2^{+} $ has been verified, no  discrete spectrum of the  $ H_2^{+} $ has been observed. However in the same thesis Wasserman reports that the ionization potential of $ H_2^{+} $ has been measured and that experimental methods show good agreement with Bates calculations.
...
The papers by Bates et all \cite{Bates1}\cite{Bates2} do not provide the numerical code used to obtain the solution. The paper states that they used the expanded fractions method, and it appears that they terminated the expansion at some point, after obtaining the desired precision. There are no details in the paper how they determined that the desired precision was achieved, whether the precision (and number of terms in the expansion) depended on the value of parameters, and so on. So the paper does not provide any details as to how the computation was performed. 

As it is customary, from now on we will be using Hartree Atomic Units \eqref{Atomic}, in all equations. In this system the numerical values of the following fundamental constants are 1:
...
Here we describe the behaviour of the Hydrogen molecular ion. The discussion here applies to both 3D and 2D cases.
...
$ \hat{H} $ represents total Hamiltonian and $ \psi\left(\mathbf{R},\mathbf{x}\right) $ is the wavefunction describing the motion of the nuclei, with coordinate (vector) $ \mathbf{R} $ and electrons with coordinate $\mathbf{x} $. Of course, since we are considering a molecular ion, the $\mathbf{x} $ is a coordinate of a single electron.
...
Kinetic energy of the 2 protons in the molecule:
\begin{equation}`
\begin{split}
...
Kinetic energy of the single electron:;
\begin{equation}
\begin{split}
...
Potential energy between the electron and the nuclei, i.e. the total Coulomb electron - nuclei attraction:
\begin{equation}
\begin{split}
...
\section{Replicating the 3D Solution for the $ H_2^{+} $ Molecular Ion}
...
Following Zygelman \cite{Zygelman89}, We expressed equations \eqref{m3d},\eqref{l3d} as a matrix eigenvalue problem. 
...
afterpage{
  ...
  \captionof{table}{Electron Energy, $ 1s\sigma_g $ state}
  \label{tab:UsVsBates}
